package io.nebula.rpc.http.autoconfigure;
...
public class HttpRpcAutoConfiguration {
    ...
    @Bean
    @ConditionalOnProperty(prefix = "nebula.rpc.http.server", name = "enabled", havingValue = "true", matchIfMissing = true)
    public RpcServiceRegistrationProcessor rpcServiceRegistrationProcessor(HttpRpcServer httpRpcServer) {
        log.info("配置RPC服务注册处理器");
