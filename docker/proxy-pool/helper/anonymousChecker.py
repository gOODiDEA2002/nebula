...
class AnonymousChecker:
    ...
    def run(self):
        ...
        # 获取本机真实出口 IP。拿不到就无法可靠判断透明代理，
        # 采取 fail-closed: 跳过本轮且不改动匿名表，避免把透明代理误当高匿写入。
        real_ip = self._get_real_ip()
        if not real_ip:
            self.log.error("未能获取本机真实 IP，跳过本轮匿名性检测(fail-closed，不修改匿名表)")
            return {"anonymous": 0, "transparent": 0, "failed": 0, "evicted": 0, "skipped": True}

        # 获取所有可用代理
        all_proxies = self.proxy_handler.getAll()
        if not all_proxies:
            self.log.info("没有可用代理，跳过匿名性检测")
            return {"anonymous": 0, "transparent": 0, "failed": 0, "evicted": 0}
        ...
        transparent_count = 0
        failed_count = 0
        passed = set()  # 本轮通过高匿检测的代理 host:port
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for future in as_completed(futures):
                ...
                if is_anonymous:
                    ...
                    passed.add(proxy.proxy)
                    anonymous_count += 1
                    self.log.debug(f"高匿代理: {proxy.proxy} -> {origin}")
                elif origin is None:
                    failed_count += 1
                    self.log.debug(f"检测失败({reason}): {proxy.proxy}")
                else:
                    transparent_count += 1
                    self.log.debug(f"{reason}: {proxy.proxy} -> {origin}")

        # 回收: 删除匿名表中本轮未通过(失效/变透明/已不在主池)的旧记录，
        # 保证 /get_anonymous 不再返回已经不安全的代理。
        evicted_count = 0
        for stale in self.proxy_handler.getAllAnonymous():
            if stale.proxy not in passed:
                self.proxy_handler.deleteAnonymous(stale)
                evicted_count += 1
        if evicted_count:
            self.log.info(f"回收失效/透明匿名代理 {evicted_count} 个")

        self.log.info(
            f"匿名性检测完成: "
            f"高匿={anonymous_count}, 透明/普通={transparent_count}, "
            f"失败={failed_count}, 回收={evicted_count}"
        )
