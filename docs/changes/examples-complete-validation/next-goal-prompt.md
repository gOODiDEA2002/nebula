...
以下内容用于继续当前的「Nebula 示例应用完全验证」Goal。Task 0 至 Task 4、Task 6 至 Task 9 已完成，
Task 5 因 OpenAI 测试账号额度不足暂时阻塞，下一阶段从 Task 10 开始。
...
本次 Goal 的第一目标：完成 tasks.md 的 Task 10，并做阶段提交；有额度的 OpenAI 密钥可用后复跑 Task 5。

Task 10 要求：
1. 不得再通过关闭 RabbitMQ、Elasticsearch、MinIO 或目标通用模块让 Fullstack 脚本通过。
2. RabbitMQ 完成发送、消费、延迟消息、失败重试和临时队列、交换机清理。
3. 使用 docker/verification/docker-compose.yml 的隔离 Elasticsearch 9.4.2，完成临时索引创建、
   单条和批量写入、查询、建议、删除，并复核索引不存在。
4. MinIO 使用 nebula-e2e 前缀临时 Bucket，完成上传、下载字节比对、预签名 URL、删除对象和 Bucket。
5. Task、Mock Payment、Mock Notification 各完成成功路径和错误输入验证。
6. 健康、性能、限流、响应缓存和脱敏端点完成严格行为断言。
7. Full 必须为 0 FAIL、0 SKIP、0 BLOCKED；清理队列、交换机、索引、Bucket、对象、缓存键、
   隔离容器、卷、端口和受管进程。
...
Task 10 完成后继续 Task 11 至 Task 16，并保留 Task 5 的复跑要求。只有 13 组示例、16 个进程、
