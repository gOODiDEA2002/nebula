...
nebula/
 ...
 infrastructure/               # 基础设施层
    ...
    messaging/                 # 消息传递
    rpc/                       # 远程调用（HTTP RPC + gRPC）
    discovery/                 # 服务发现（Nacos）
    websocket/                 # WebSocket（Spring + Netty 双实现）
    lock/                      # 分布式锁（Redisson）
...
> **注意**：推荐使用 Starter 而非直接依赖 `nebula-autoconfigure`。各 Starter 已通过
> `NebulaStarterDefaultsPostProcessor` 自动启用对应模块，无需重复声明 `enabled: true`。
> 以下示例适用于直接使用 `nebula-autoconfigure` 的场景。
...
所有配置的功能模块将自动初始化并可用。
如果使用 Starter（如 `nebula-starter-service`），其 `nebula-defaults.properties`
已默认启用对应模块，只需配置连接信息（server-addr 等）即可。
...
根据项目类型选择合适的 Starter，引入即可开箱即用：

| Starter | 适用场景 | 默认启用模块 |
|---------|---------|-------------|
| `nebula-starter-web` | REST API / 管理后台 | Persistence, Cache |
...
| `nebula-starter-minimal` | CLI / 批处理 | 无 |

#### 1. 添加依赖
```xml
<!-- Web 应用选择 nebula-starter-web -->
<dependency>
    <groupId>io.nebula</groupId>
...
> 各 Starter 通过 `NebulaStarterDefaultsPostProcessor` 自动启用对应模块，
> 无需在 `application.yml` 中重复声明 `enabled: true`。

### 方式二：使用 nebula-autoconfigure（精确控制）

适合需要精确控制依赖的场景，需手动声明每个模块的 `enabled: true`：
...
**使用 Starter 时**：Starter 已自动启用对应模块，只需配置连接参数：
...
**使用 nebula-autoconfigure 时**：需显式声明每个模块的 `enabled: true`：

```yaml
nebula:
  data:
    persistence:
      enabled: true   # Level 2，默认禁用
    cache:
      enabled: true   # Level 2，默认禁用
  messaging:
    rabbitmq:
...
- Security（Level 1）默认启用，其他模块默认禁用
- 使用 Starter 时对应模块自动启用，使用 autoconfigure 时需显式配置
