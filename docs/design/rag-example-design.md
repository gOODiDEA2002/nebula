...
- **F-4 推理模型正文为空未判降级（记录，未改框架）**：Hy3 思考 token 计入 `max_tokens`，`256` 时 `finish_reason=length` 且 content 为空，`RagPipeline` 返回 `answer=""` 且 `degraded=false`，流式路径为 127 个空 `DELTA`。示例侧改默认 `max-tokens=2048` 解决；框架可考虑把「空正文 / finish_reason=length」计入降级原因，另立任务。
- **F-5 `vectorStoreIndexSink` 无 `@Order`（记录，未改框架）**：多写目标顺序不确定，示例 E2E 的日志断言已按顺序无关写法处理；若后续要求「向量库先写、关键词后写」需框架补 `@Order`。
- **F-1 `rerank.http.url` 空值即装配**：`HttpRerankConfiguration` 用 `@ConditionalOnProperty(name = "url")`，Spring 对该写法的判定是「键存在且不为 false」，空字符串也算存在。用户若写 `url: ${RERANK_HTTP_URL:}` 占位，会装配出指向空地址的 HTTP 重排器，每次重排都走直通并刷 warn。R2 的 `search.index-name` 已用自定义非空条件规避同类问题，建议 `rerank.http.url` 对齐。本示例用注释而非空占位规避，README 提醒。属框架小修，另立任务。
