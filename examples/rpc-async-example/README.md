...
```java
// 客户端接口
@RemoteService("data-process-service")
...
```bash
mvn -q -f examples/rpc-async-example/service spring-boot:run
```

**启动客户端（端口 8082）：**

```bash
mvn -q -f examples/rpc-async-example/client spring-boot:run
```
...
- [nebula-rpc-async 框架文档](../../infrastructure/rpc/nebula-rpc-async/README.md)
- [微服务 RPC 示例](../starter-service-example/README.md)
- [微服务拆分示例](../microservice-example/README.md)
